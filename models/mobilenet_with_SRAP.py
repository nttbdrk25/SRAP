...
from .SRAP_Attention import *

###
#%% MobileNet building blocks
###

class DepthwiseSeparableConvBlock(torch.nn.Module):#pointwise_attention
    """
    ...
    def __init__(self,
        ...
        self.TOPIMAP = SRAP(in_channels, pool_types=pool_types,rate=rate)
    def forward(self, x):
...
class LinearBottleneck(torch.nn.Module): #pointwise_attention
    """
    ...
    def __init__(self,
                 ...
                 activation="relu6",
                 kernel_size=3,
                 use_se=False,pool_types=None,rate=None):
        super().__init__()
        self.use_res_skip = (in_channels == out_channels) and (stride == 1)
        self.use_se = use_se
        ...
        if kernel_size == 3:
            ...
            raise ValueError
        if self.use_se:
            self.TOPIMAP = SRAP(mid_channels, pool_types=pool_types,rate=rate)
...
class MobileNetV1(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 in_channels=3,
                 in_size=(224, 224),
                 use_data_batchnorm=True,pool_types=None,rate=None):
        super().__init__()
        ...
        for stage_id, stage_channels in enumerate(channels):
            ...
            for unit_id, unit_channels in enumerate(stage_channels):
                ...
                stage.add_module("unit{}".format(unit_id + 1), DepthwiseSeparableConvBlock(in_channels=in_channels, out_channels=unit_channels, stride=stride,pool_types=pool_types,rate=rate))
    ...
    def __init__(self,
        ...
        for stage_id, stage_channels in enumerate(channels):
            ...
            for unit_id, unit_channels in enumerate(stage_channels):
                ...
                stage.add_module("unit{}".format(unit_id + 1), LinearBottleneck(in_channels=in_channels, mid_channels=mid_channel, out_channels=unit_channels, stride=stride,use_se=True,pool_types=pool_types,rate=rate))
                in_channels = unit_channels
            self.backbone.add_module("stage{}".format(stage_id + 1), stage)
    ...
    def __init__(self,
        ...
        for stage_id, stage_channels in enumerate(channels):
            ...
            for unit_id, unit_channels in enumerate(stage_channels):
                ...
                stage.add_module("unit{}".format(unit_id + 1), LinearBottleneck(in_channels=in_channels, mid_channels=mid_channel, out_channels=unit_channels, stride=stride, activation=activation, kernel_size=kernel_size,use_se=use_se,pool_types=pool_types,rate=rate))
                in_channels = unit_channels
            self.backbone.add_module("stage{}".format(stage_id + 1), stage)
        ...
        if final_conv_se:
            self.backbone.add_module("final_topkFeatures", BAMM(out_channels, pool_types=pool_types,rate=rate))   
...
def build_mobilenet_v1(num_classes, width_multiplier=1.0, cifar=False,pool_types=None,rate=None):
    ...
    return MobileNetV1(num_classes=num_classes,
                       ...
                       channels=channels,
                       strides=strides,
                       in_size=in_size,pool_types=pool_types,rate=rate)


def build_mobilenet_v2(num_classes, width_multiplier=1.0, cifar=False,pool_types=None,rate=None):
    """
...
def build_mobilenet_v3(num_classes, version, width_multiplier=1.0, cifar=False, use_lightweight_head=True,pool_types=None,rate=None):
    """
