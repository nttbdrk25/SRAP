...
from models.mobilenet_with_SRAP import *
...
def get_args():
    ...
    """
    parser = argparse.ArgumentParser(description='mobilenet training script for CIFAR and fine-grained datasets.', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
def main():
    ...
    #arr_model = ['mobilenetv1','mobilenetv2','mobilenetv3']    
    arr_model = ['mobilenetv3']    
    multiplier = 1.0        
    pool_types=['avg','std']   
    for idm in arr_model:                
        id_rate = [0.5,0.5]                
        strmode = idm +'_SRAP'+pool_types[0]+pool_types[1] + str(multiplier) + '_' + str(id_rate[0]) + '_' + str(id_rate[1])        
        pathout = './checkpoints/Dogs_SRAP/' + strmode
        filenameLOG = pathout + '/' + strmode + '.txt'
        if not os.path.exists(pathout):
            os.makedirs(pathout)
        # get model        
        if idm == 'mobilenetv1':
            model = build_mobilenet_v1(120, width_multiplier=multiplier, cifar=False,pool_types=pool_types,rate=id_rate)
        if idm == 'mobilenetv2':
            model = build_mobilenet_v2(120, width_multiplier=multiplier, cifar=False,pool_types=pool_types,rate=id_rate)
        if idm == 'mobilenetv3':
            model = build_mobilenet_v3(120, "large", width_multiplier=multiplier, cifar=False, use_lightweight_head=False,pool_types=pool_types,rate=id_rate)            
        ...
        if args.evaluate:
            pathcheckpoint = "./checkpoints/StanfordDogs/model_best.pth"
            if os.path.isfile(pathcheckpoint):
                print("=> loading checkpoint '{}'".format(pathcheckpoint))
                checkpoint = torch.load(pathcheckpoint,map_location="cuda:0")
